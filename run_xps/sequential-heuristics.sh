...
GLOBAL_CONTEXT_TYPES=("left" "right" "neighbors")
...
for TASK_NAME in "${SELECTED_TASKS[@]}"; do
    ...
    for CONTEXT_TYPE in "${GLOBAL_CONTEXT_TYPES[@]}"; do
        ...
        START_TIME=$(date +%s)
        python "$SCRIPT_PATH" --input_dir "$TASK_DIR" --output_dir "$TASK_OUTPUT_DIR" --k_min $K_MIN --k_max $K_MAX --context_type $CONTEXT_TYPE
        END_TIME=$(date +%s)
