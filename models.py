...
class CRFPerTaskOutputLayer(torch.nn.Module):
    ...
    def forward(self, task, x, mask, labels=None, output_all_tasks=False):
        ...
        # print(f"output_all_tasks: {output_all_tasks}")
...
class BertHSLN(torch.nn.Module):
    ...
    def forward(self, batch, labels=None, output_all_tasks=False):
        ...
        target_sentences_mask = batch["target_sentences"]  # Shape: (documents, sentences)
        target_sentences_mask = target_sentences_mask.unsqueeze(-1).expand(-1, -1,
                                                                           sentence_embeddings_encoded.shape[-1])
        target_sentence_embeddings = sentence_embeddings_encoded * target_sentences_mask
        target_sentence_embeddings = target_sentence_embeddings.sum(dim=1)  # Shape: (documents, embedding_dim)

        # print(target_sentence_embeddings.shape)

        # Debugging: Inspect sentence embeddings and target sentence embeddings
        # print(f"Target Sentences Mask:\n{batch['target_sentences']}")
        #
        # print(f"\n[DEBUG] Sentence Embeddings Shape: {sentence_embeddings_encoded.shape}")
        #
        # print(f"Target Sentence Embeddings Shape: {target_sentence_embeddings.shape}")

        # Debugging: Inspect labels before filtering
        if labels is not None:
            # print(f"[DEBUG] Original Labels Shape: {labels.shape}")
            # print(f"[DEBUG] Original Labels: {labels}")

            # Select labels corresponding to the target sentence
            target_sentences_mask_flat = batch["target_sentences"]  # Shape: (documents, sentences)
            filtered_labels = labels * target_sentences_mask_flat  # Mask non-target labels
            filtered_labels = filtered_labels.sum(dim=1).unsqueeze(1)  # Shape: (documents, 1)

            # print(f"[DEBUG] Filtered Labels Shape: {filtered_labels.shape}")
            # print(f"[DEBUG] Filtered Labels: {filtered_labels}")

            labels = filtered_labels

        # Classification on the target sentence embeddings
        if self.generic_output_layer:
            output = self.crf(target_sentence_embeddings.unsqueeze(1), batch["sentence_mask"][:, :1], labels)
        else:
            output = self.crf(batch["task"], target_sentence_embeddings.unsqueeze(1), batch["sentence_mask"][:, :1],
                              labels, output_all_tasks)
